...
def check_local_server():
    ...
    for url in urls_to_check:
        try:
            ...
            print(f"✅ {url} - 상태: {response.status_code}")
            return True
        except:
            print(f"❌ {url} - 연결 실패")
    return False
...
def check_local_network():
    ...
    if local_ip:
        ...
        try:
            response = requests.get(url, timeout=5)
            print(f"✅ 로컬 네트워크 접근 성공: {url} - {response.status_code}")
            return True
        except Exception as e:
            print(f"❌ 로컬 네트워크 접근 실패: {url} - {e}")
    return False
    ...
    try:
        if sys.platform.startswith('win'):
            ...
            if ':8000' in result.stdout:
                print("✅ 포트 8000이 열려있음")
                return True
        print("❌ 포트 8000이 열려있지 않음")
        return False
    except:
        print("⚠️ 포트 상태를 확인할 수 없음")
        return False
...
def test_recommend_endpoint():
    ...
    try:
        ...
        print(f"✅ 로컬 recommend 엔드포인트: {response.status_code}")
        if response.status_code == 200:
            print(f"   응답: {response.json()}")
        return True
    except Exception as e:
        print(f"❌ recommend 엔드포인트 실패: {e}")
        return False

def main():
    print("🔍 FastAPI 서버 디버깅 시작")
    print("="*50)
